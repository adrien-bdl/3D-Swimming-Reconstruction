...
def save_frames_single_camera(camera_name):
	...
	#get settings
	camera_device_id = calibration_settings[camera_name]['id']
	width = calibration_settings[camera_name]['frame_width']
	height = calibration_settings[camera_name]['frame_height']
	...
	while True:
		...
		frame_small = cv.resize(frame,
		                        None,
		                        fx=1 / view_resize,
		                        fy=1 / view_resize)

		if not start:
			cv.putText(frame_small,
			           "Press SPACEBAR to start collection frames", (50, 50),
		...
		if start:
			cooldown -= 1
			cv.putText(frame_small,
			           "Cooldown: " + str(cooldown), (50, 50),
			           cv.FONT_HERSHEY_COMPLEX,
			           1, (0, 255, 0),
			           1)
			cv.putText(frame_small,
			           "Num frames: " + str(saved_count), (50, 100),
			           cv.FONT_HERSHEY_COMPLEX,
			           1, (0, 255, 0),
			           1)
...
def calibrate_camera_for_intrinsic_parameters(images_prefix):
	...
	rows = calibration_settings[
	    'checkerboard_rows'] - 1  # le problème des clôtures et des piquets
	columns = calibration_settings[
	    'checkerboard_columns'] - 1  # le problème des clôtures et des piquets
	world_scaling = calibration_settings[
	    'checkerboard_box_size_scale']  #this will change to user defined length scale
	...
	for i, frame in enumerate(images):
		...
		if not ret:
			print("Could not find checkerboard in image: ", images_names[i])
		else:
...
def save_camera_intrinsics(camera_matrix, distortion_coefs, camera_name):
	...
	out_filename = os.path.join('camera_parameters',
	                            camera_name + '_intrinsics.dat')
...
def save_frames_two_cams(camera0_name, camera1_name):
	...
	cap0 = cv.VideoCapture(calibration_settings[camera0_name]['id'])
	cap1 = cv.VideoCapture(calibration_settings[camera1_name]['id'])

	#set camera resolutions
	cap0.set(3, calibration_settings[camera0_name]['frame_width'])
	cap0.set(4, calibration_settings[camera0_name]['frame_height'])
	cap1.set(3, calibration_settings[camera1_name]['frame_width'])
	cap1.set(4, calibration_settings[camera1_name]['frame_height'])
	...
	while True:
		...
		frame0_small = cv.resize(frame0,
		                         None,
		                         fx=1. / view_resize,
		                         fy=1. / view_resize)
		frame1_small = cv.resize(frame1,
		                         None,
		                         fx=1. / view_resize,
		                         fy=1. / view_resize)
		...
		if not start:
			cv.putText(
			    ...
			    1, (0, 0, 255),
			    1)
			cv.putText(frame0_small,
			           "Press SPACEBAR to start collection frames", (50, 100),
			           cv.FONT_HERSHEY_COMPLEX,
			           1, (0, 0, 255),
			           1)
		...
		if start:
			...
			if cooldown <= 0:
				savename = os.path.join(
				    'frames_pair',
				    camera0_name + '_' + str(saved_count) + '.png')
				cv.imwrite(savename, frame0)

				savename = os.path.join(
				    'frames_pair',
				    camera1_name + '_' + str(saved_count) + '.png')
...
def stereo_calibrate(mtx0,
	...
	#frame dimensions. Frames should be the same size.
	width = min(c0_images[0].shape[1], c1_images[0].shape[1])
	height = min(c0_images[0].shape[0], c1_images[0].shape[0])
	...
	for i, (frame0, frame1) in enumerate(zip(c0_images, c1_images)):
		cv.resize(frame0, (width, height))
		gray1 = cv.cvtColor(frame0, cv.COLOR_BGR2GRAY)
		cv.resize(frame1, (width, height))
		gray2 = cv.cvtColor(frame1, cv.COLOR_BGR2GRAY)
		c_ret1, corners1 = cv.findChessboardCorners(gray1, (rows, columns), None)
		c_ret2, corners2 = cv.findChessboardCorners(gray2, (rows, columns), None)

		if not (c_ret1 and c_ret2):
			if not c_ret1:
				print("Could not find checkerboard in image: ",
				      c0_images_names[i])
			if not c_ret2:
				print("Could not find checkerboard in image: ",
				      c1_images_names[i])
		else:
			corners1 = cv.cornerSubPix(gray1,
			                           corners1, (11, 11), (-1, -1),
			                           criteria)
			corners2 = cv.cornerSubPix(gray2,
			                           corners2, (11, 11), (-1, -1),
			                           criteria)
...
def check_calibration(camera0_name,
                      camera0_data,
                      camera1_name,
                      camera1_data,
                      _zshift=50.):
	...
	coordinate_points = np.array([[0., 0., 0.], [1., 0., 0.], [0., 1., 0.],
	                              [0., 0., 1.]])
...
def get_cam1_to_world_transforms(cmtx0,
                                 dist0,
                                 R_W0,
                                 T_W0,
                                 cmtx1,
	...
	unitv_points = 5 * np.array([[0, 0, 0], [1, 0, 0], [0, 1, 0], [0, 0, 1]],
...
def save_extrinsic_calibration_parameters(R0, T0, R1, T1, prefix=''):
	...
	camera0_rot_trans_filename = os.path.join('camera_parameters',
	                                          prefix + 'camera0_rot_trans.dat')
	...
	camera1_rot_trans_filename = os.path.join('camera_parameters',
	                                          prefix + 'camera1_rot_trans.dat')
...
if __name__ == '__main__':
	...
	if len(sys.argv) == 1:
		sys.argv.append('calibration_settings.yaml')
	elif len(sys.argv) != 2:
		print(
		    'Call with settings filename: "python3 calib.py calibration_settings.yaml"'
		)
		quit()
	...
	steps_to_be_performed = {
	    1: True, 2: True, 3: True, 4: True, 5: True, "optional": False
	}

	if steps_to_be_performed[1]:
		print("""Step1. Save calibration frames for single cameras""")
		print("save frames for camera0")
		# Remove all files from the folder frames
		...
		print("save frames for camera1")
		save_frames_single_camera('camera1')

	if steps_to_be_performed[2]:
		print("""Step2. Obtain camera intrinsic matrices and save them""")
		#camera0 intrinsics
		...
		save_camera_intrinsics(
		    cmtx0, dist0, 'camera0')  #this will write cmtx and dist to disk
		#camera1 intrinsics
		images_prefix = os.path.join('frames', 'camera1*')
		cmtx1, dist1 = calibrate_camera_for_intrinsic_parameters(images_prefix)
		save_camera_intrinsics(
		    cmtx1, dist1, 'camera1')  #this will write cmtx and dist to disk

	if steps_to_be_performed[3]:
		print(
	...
	if steps_to_be_performed[5]:
		...
		check_calibration('camera0',
		                  camera0_data,
		                  'camera1',
		                  camera1_data,
		                  _zshift=60.)

	if steps_to_be_performed["optional"]:
		print(
		...
		save_extrinsic_calibration_parameters(
		    R_W0, T_W0, R_W1, T_W1,
		    prefix='world_to_')  #this will write R and T to disk
